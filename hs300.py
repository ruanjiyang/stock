...
x=np.zeros([600-20+1,20,13])  #开始构造训练集x，30组，每组20天，13个特征值）。按照每天来滑动x的窗口。
for i in range(totalHistDay-20+1):
    for j in range(20):
        x[i][j]=newHS300DataHist[i+j-1]

print(x[580])

y=np.zeros([600-20+1,13])
for i in range(totalHistDay-20+1):
    for j in range(13):
        y[i][j]=newHS300DataHist[i+20-1][j]
print(y[580])

# y=np.zeros([600-20+1,20,13])

# y=np.zeros([600-20+1,1])
